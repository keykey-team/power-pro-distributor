...
const ProductItem = ({ product, onDeleteClick }) => {
    ...
    const title = product?.title?.ua || product?.title?.en || product?.title?.ru || product?.title?.sk || product?.slug || "Назва"
    ...
    return (
        <div className='product-item products-grid'>
            ...
            <div className="product-item__func">
                ...
                <button type="button" onClick={() => onDeleteClick(product._id)} style={{ color: 'var(--color-danger)' }}>
                ...
                <button type="button" onClick={() => putchClick(product._id, navigate)} style={{ color: 'var(--color-success-text)' }}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 15 15" fill="none">
