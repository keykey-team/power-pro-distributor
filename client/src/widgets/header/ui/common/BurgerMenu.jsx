...
const BurgerMenu = () => {
    ...
    // Отримання поточної локалі з URL
    const segments = pathname.split('/').filter(Boolean);
    const currentLocale = segments[0] || "sk";
 const params = useParams();
    const locale = params.locale;
    // Функція зміни мови з перенаправленням
    const onSelectLanguage = (nextLocale) => {
    ...
    // Закриття меню та скрол
    const handleNavClick = (anchor) => {
        scrollToElement(locale,anchor);
        setIsModalOpen(null);
    };
        ...
        <>
            ...
            <div className={isModalOpen === "burger" ? "burger-menu active" : "burger-menu"}>
                <div className="burger-menu__list">
                    {/* Порожній блок з вашого коду */}
                </div>

                <ul className="burger-menu__nav">
                    <li onClick={() => handleNavClick("prev")}>
                        <p>{t("navigation.header.us")}</p>
                    </li>
                    <li onClick={() => handleNavClick("partners")}>
                        <p>{t("navigation.header.partners")}</p>
                    </li>
                    <li onClick={() => handleNavClick("prods")}>
                        <p>{t("navigation.header.prods")}</p>
                    </li>
                    <li onClick={() => handleNavClick("order-form")}>
                ...
                <div className="burger-menu__language">
                    <div className="language">
                        {LANGS.map((lang) => (
                            <div 
                                key={lang.locale}
                                className={`language-main ${currentLocale === lang.locale ? "active" : ""}`}
                ...
                <p className='burger-menu__contact'>
                ...
                <button 
                    ...
                    onClick={() => { router.push(`/${locale}/build-box`); setIsModalOpen(null); }}
                >
                    {t("box")}
                </button>
            </div>
