            ...
            <div className={isModalOpen === 'curt' ? 'curt active' : 'curt'}>
                ...
                <div className="curt__list">
                    {cart.length === 0 ? (
                        ...
                        <ul>
                            {cart.map((item, index) => (
                                <li
                                    key={index} // ← исправлено: вместо item.id используем productId
                                    className="curt__item"
                                    onClick={() => {
                                ...
                                >
                                    <div className="curt__item-content">
                                        <CartImage
                                            src={item?.product?.gallery?.[0] || item?.product?.cover}
                                            alt={item.name}
                                        />
                                    ...
                                    <div className="curt__item-quantity">
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                updateQuantity(item.productId, -1); // теперь работает
                                            }}
                                        >
                ...
                <div className="curt__func">
                    <div className="curt__func-data">
                        <p>{t('cart.cost')}</p>
                        <p>
                            {total.toFixed(2)} {t('cart.currency') || 'грн'}
                        </p>
                    </div>
                    <div
                        className={cart.length === 0 ? "curt__func-button disable" : "curt__func-button"}
